...
function ViewportImageSliceLoadingIndicator({ viewportData, element }) {
  ...
  if (error) {
    return (
      <>
        <div className="bg-background/70 absolute top-0 left-0 h-full w-full">
  ...
  if (loading) {
    return (
      ...
      <div className="bg-background/70 pointer-events-none absolute top-0 left-0 h-full w-full">
        <div className="transparent flex h-full w-full items-center justify-center">
          <p className="text-foreground text-xl font-light">Loading...</p>
        </div>
      </div>
    );
  }
