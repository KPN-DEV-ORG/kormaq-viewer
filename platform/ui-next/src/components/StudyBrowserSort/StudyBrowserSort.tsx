      ...
      <DropdownMenu>
        <Tooltip>
          <TooltipTrigger className="w-full overflow-hidden">
            <DropdownMenuTrigger className="border-inputfield-main focus:border-inputfield-main flex h-[26px] w-full items-center justify-start overflow-hidden whitespace-nowrap rounded border bg-background p-2 text-base text-foreground">
              {selectedSort.label}
            </DropdownMenuTrigger>
          </TooltipTrigger>
        ...
        <TooltipTrigger>
          ...
          >
            {sortDirection === 'ascending' ? (
              <Icons.SortingAscending className="text-highlight w-2" />
            ) : (
              <Icons.SortingDescending className="text-highlight w-2" />
            )}
          </button>
        </TooltipTrigger>
